...
function classifyHeroes(name,xpQuantity){
    ...
    if(xpQuantity >= 100001){
        ...
        return `Olá ${name}! Seu nível é Ascendente`;
    }else if( xpQuantity >= 7001 && xpQuantity <= 8000){
        return `Olá ${name}! Seu nível é Platina `;
    }else if( xpQuantity >= 6000 && xpQuantity <= 7000){
        return `Olá ${name}! Seu nível é Ouro`;
    }else if( xpQuantity >= 2001 && xpQuantity <=5999){
        return `Olá ${name}! Seu nível é Prata`;
    }else if(xpQuantity >= 1001 && xpQuantity <= 2000 ){
        return `Olá ${name}! Seu nível é Bronze`;
    }else {
        return `Olá ${name}! Seu nível é Ferro`;
    }
}
